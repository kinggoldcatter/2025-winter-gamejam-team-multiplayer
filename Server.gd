...
func _process(delta):
	...
	if peer.get_available_packet_count() > 0:
		...
		if packet != null:
			...
			elif data.message ==  Utilities.Message.offer || data.message ==  Utilities.Message.answer || data.message ==  Utilities.Message.candidate:
				print("source id is " + str(data.orgPeer))
				sendToPlayer(data.peer, data)
				
			elif data.message ==  Utilities.Message.removeLobby:
